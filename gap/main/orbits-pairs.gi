...
InstallGlobalFunction(OrbitsOfPairs, function(G, S, act)
  local join, part, rep, x, k, p, J, lParts, addElements, Ag, I, T, tLength, A, GS, D, R, L, invPartition, invPartUF, invParts, i, j, pairs, g, y, IsDone;
  ...
  Next := function(self)
    ...
    while true do
      while self!.pos < Length(self!.currentList) do
        ...
        if self!.i = self!.j then
          if self!.ti <> act(self!.ti, g) then
            return [[act(self!.ti, g), self!.ti], Size(GS) / SIZE_STABILISER_PAIR_CONJUGATE(A[self!.i], self!.ti, g, self!.act)];
          fi;
        else
          return [[act(self!.ti, g), self!.tj], Size(GS) / Size(Intersection(self!.act(A[self!.i], g), A[self!.j]))];
        fi;
      od;
      self!.currentList := [];
      self!.pos := 0;
